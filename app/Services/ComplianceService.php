...
class ComplianceService
{
    ...
    protected function checkRequiredDocuments(Vendor $vendor, ComplianceRule $rule): array
    {
        ...
        return ['status' => ComplianceResult::STATUS_FAIL, 'details' => 'Missing mandatory documents: ' . implode(', ', $missingNames), 'metadata' => ['missing_document_ids' => $missingTypes->toArray()]];
    }
    /**
    ...
    protected function checkDocumentExpiry(Vendor $vendor, ComplianceRule $rule): array
    {
        ...
        if ($expiredDocs->isNotEmpty()) {
            return ['status' => ComplianceResult::STATUS_FAIL, 'details' => 'Expired documents: ' . $expiredDocs->pluck('documentType.display_name')->implode(', '), 'metadata' => ['expired_document_ids' => $expiredDocs->pluck('id')->toArray()]];
        }
        if ($soonToExpire->isNotEmpty()) {
            return ['status' => ComplianceResult::STATUS_WARNING, 'details' => 'Documents expiring soon: ' . $soonToExpire->pluck('documentType.display_name')->implode(', '), 'metadata' => ['expiring_document_ids' => $soonToExpire->pluck('id')->toArray()]];
        }
